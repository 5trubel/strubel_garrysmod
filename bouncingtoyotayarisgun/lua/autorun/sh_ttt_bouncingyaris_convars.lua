...
local function CreateUI(conv_list)
	if CLIENT then
		hook.Add('TTTUlxModifyAddonSettings', 'ttt_bouncingyaris_hook_to_TTT_ULX_UI', function(name)
--# create BAse Layout
...
--# add each ConVar to UI_conv_list
		...
		ui_conv_list:AddItem(xlib.makelabel{label ="Bouncing Yaris by Strub3l",textcolor = Color( 0, 0, 180),parent = ui_conv_list})
--# add to ULX
		xgui.hookEvent('onProcessModules', nil, thomas_panel.processModules)
		xgui.addSubModule('Bouncing Yaris', thomas_panel, nil, name)
		end)
	end
end
local function CustomULXConVars()
	---DEBUG print("###### Bouncing Yaris CONVAR start #######")
        AddonConVar("ttt_bouncingyaris_det_stop",1,"bool",true,true,true,"Detective stops Bouncing Yaris")
		AddonConVar("ttt_bouncingyaris_det_stop_say",1,"bool",true,true,true,"Detective tells chat if he stoped Bouncing Yaris")
		AddonConVar("ttt_bouncingyaris_det_survive",1,"bool",true,true,true,"Detective survives Bouncing Yaris")
		AddonConVar("ttt_bouncingyaris_speed",350,"int",true,true,true,"How fast should Bouncing Yaris go?",0,1000,0)
		AddonConVar("ttt_bouncingyaris_explode_time",15,"int",true,true,true,"Seconds till Bouncing Yaris explodes:",0,60,0)
		AddonConVar("ttt_bouncingyaris_explode_magnitude",256,"int",true,true,true,"Magnitude of explosion:",0,512,0)
		AddonConVar("ttt_bouncingyaris_explode_radius",512,"int",true,true,true,"Radius of explosion:",0,1024,0)
	---DEBUG print("###### Bouncing Yaris CONVAR END #########")
	
    hook.Add('TTTUlxInitCustomCVar', 'ttt_bouncingyaris_hook_to_TTT_ULX_ConVar', function(ulib_acess)
